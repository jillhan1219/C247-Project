...
class RNNCTCModule(pl.LightningModule):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Select Encoder based on rnn_type
        encoder_class = GRUEncoder if rnn_type.lower() == "gru" else LSTMEncoder
        ...
        self.model = nn.Sequential(
            ...
            encoder_class(in_features=num_features, hidden_dim=hidden_dim, num_layers=num_layers, dropout=dropout),
    ...
    def _step(self, phase: str, batch: dict[str, torch.Tensor], *args, **kwargs) -> torch.Tensor:
        ...
        predictions = self.decoder.decode_batch(emissions.detach().cpu().numpy(), emission_lengths.detach().cpu().numpy())
        metrics = self.metrics[f"{phase}_metrics"]
        targets_np, target_lengths_np = targets.detach().cpu().numpy(), target_lengths.detach().cpu().numpy()
        for i in range(N):
            metrics.update(prediction=predictions[i], target=LabelData.from_labels(targets_np[:target_lengths_np[i], i]))
